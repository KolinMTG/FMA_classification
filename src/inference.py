"""Module for music genre inference from audio files and TFRecord datasets."""
...
def load_config(normalization_stats_path: str, label_mapping_path: str) -> Dict:
    ...
    if not os.path.exists(normalization_stats_path):
        raise FileNotFoundError(
            f"Normalization stats not found: {normalization_stats_path}"
        )
    ...
    logger.info(
        f"Configuration loaded: {len(config['label_mapping'])} genres, "
...
def load_tfrecord_segments(
    tfrecord_path: str, model_input_shape: Tuple[int, int, int]
) -> np.ndarray:
    """
    ...
    for raw_record in dataset:
        ...
        spectrogram = tf.reshape(
            spectrogram, [target_height, target_width, target_channels]
        )
...
def split_into_segments(
    waveform: np.ndarray, sample_rate: int, segment_duration: float, overlap: float
) -> List[np.ndarray]:
...
def apply_normalization(
    log_mel: np.ndarray, mean: np.ndarray, std: np.ndarray, per_bin: bool
) -> np.ndarray:
...
def handle_spectrogram_shape(
    ...
    if current_height != target_height:
        raise ValueError(
            f"Frequency dimension mismatch: got {current_height}, expected {target_height}"
        )
...
def process_segments(
    segments: List[np.ndarray], config: Dict, target_shape: Tuple[int, int]
) -> np.ndarray:
    ...
    for segment in segments:
        ...
        normalized = apply_normalization(
            log_mel, config["mean"], config["std"], config["per_bin"]
        )
...
def predict_from_audio(
    ...
    segments = split_into_segments(
        waveform, config["sample_rate"], segment_duration, overlap
    )
    ...
    predicted_label = config["label_mapping"].get(
...
def aggregate_predictions(
    predictions: np.ndarray, method: str
...
def get_top_n_predictions(
    probabilities: np.ndarray, label_mapping: Dict[int, str]
) -> List[Dict[str, any]]:
    """
...
def save_prediction_report(
    results: Dict, label_mapping: Dict[int, str], output_path: str
) -> None:
    ...
    try:
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"Confidence: {results['confidence']:.4f} ({results['confidence']*100:.2f}%)\n"
            )
...
def predict_single_file(
    ...
    try:
        if file_ext == ".tfrecord":
            result = predict_from_tfrecord(
                model, label_mapping, file_path, aggregation_method
            )
        elif file_ext in [".mp3", ".wav", ".flac", ".ogg"]:
            if config is None:
                raise ValueError("config required for audio file processing")
            result = predict_from_audio(
                model, config, file_path, segment_duration, overlap, aggregation_method
            )
        else:
...
def predict_folder(
    ...
    logger.info(
        f"Found {len(audio_files)} audio files, {len(tfrecord_files)} TFRecord files"
    )
...
def predict_genre(
    ...
    config = load_config(normalization_stats_path, label_mapping_path)
    result = predict_from_audio(
        model, config, audio_path, segment_duration, overlap, aggregation_method
    )
