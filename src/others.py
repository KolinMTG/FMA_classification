"""Miscellaneous utility functions for file management, data suppression/restoration, and other helper tasks."""
...
def suppress_useless(
    suppress_path_list: List[str],
    historic_name: str = HISTORIC_TRASH_CSV_NAME,
    target_folder_path: str = TRASH_DIR,
) -> None:
    """
    Args:
    ...
    with historic_path.open("a", newline="", encoding="utf-8") as csvfile:
        ...
        for path_str in suppress_path_list:
            ...
            writer.writerow(
                [str(original_path), str(moved_path), original_path.is_dir()]
            )
...
def restore_data(
    historic_name: str = HISTORIC_TRASH_CSV_NAME, source_folder_path: str = TRASH_DIR
) -> None:
    """
    Args:
        historic_name (str): Name of the historic CSV file.
...
if __name__ == "__main__":
    ...
    show_spectrogram_from_tfrecord(
        tfrecord_path=r"data\tfrecords_32\test\000002.tfrecord", index=0
    )
