...
def example_2_standard_run(tfrecord_dir=TFRECORD_OUTPUT_DIR_64):
    ...
    print(
        f"  - {config.population_size * config.num_generations} neuroevolution models"
    )
...
def analyze_results(results_dir="results_standard"):
    ...
    top_5 = df.nlargest(5, "val_accuracy")[
        ["model_id", "generation", "val_accuracy", "architecture", "model_path"]
    ]
    ...
    if "model_path" in df.columns:
        ...
        # Verify files exist
        existing = saved_models["model_path"].apply(
    ...
    if "conv_layers" in df.columns:
        df["num_conv_layers"] = df["conv_layers"].apply(
            lambda x: len(eval(x)) if pd.notna(x) else 0
        )
        df["num_dense_layers"] = df["dense_layers"].apply(
            lambda x: len(eval(x)) if pd.notna(x) else 0
        )
    ...
    for idx, row in df.iterrows():
        if pd.notna(row["model_path"]):
            if not os.path.exists(row["model_path"]):
                missing.append({"model_id": row["model_id"], "path": row["model_path"]})
