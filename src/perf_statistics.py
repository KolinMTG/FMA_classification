"""Module for analyzing and visualizing model performance statistics. Generate plots and reports and add CSV."""
...
class ModelPerformanceAnalyzer:
    ...
    def analyze_class_distribution(self):
        ...
        true_dist = (
            self.df_success["true_label"]
            .value_counts()
            .reindex(self.classes, fill_value=0)
        )
        pred_dist = (
            self.df_success["predicted_label"]
            .value_counts()
            .reindex(self.classes, fill_value=0)
        )
        ...
        dist_df = pd.DataFrame(
            {"True_Label_Count": true_dist, "Predicted_Label_Count": pred_dist}
        )
    ...
    def analyze_per_class_performance(self):
        ...
        per_class_df.to_csv(
            os.path.join(self.tables_dir, "per_class_metrics.csv"), index=False
        )
    ...
    def analyze_confidence_scores(self):
        ...
        ax.axvline(
            self.df_success["confidence"].mean(),
            color="red",
            linestyle="--",
            label=f'Mean: {self.df_success["confidence"].mean():.3f}',
        )
        ax.axvline(
            self.df_success["confidence"].median(),
            color="green",
            linestyle="--",
        ...
        ax.set_title(
            "Overall Confidence Score Distribution", fontsize=14, fontweight="bold"
        )
        ax.legend()
        plt.tight_layout()
        ...
        self.df_success.boxplot(
            column="confidence", by="predicted_label", ax=ax, patch_artist=True
        )
        ax.set_xlabel("Predicted Class", fontsize=12)
        ax.set_ylabel("Confidence Score", fontsize=12)
        ax.set_title(
            "Confidence Distribution by Predicted Class", fontsize=14, fontweight="bold"
        )
        ...
        self.df_success["correct"] = (
            self.df_success["true_label"] == self.df_success["predicted_label"]
        )
        ...
        conf_stats = pd.DataFrame(
            {
                "Overall": [
                    self.df_success["confidence"].mean(),
                    self.df_success["confidence"].std(),
                    self.df_success["confidence"].min(),
                    self.df_success["confidence"].max(),
                ],
                "Correct": [
                    correct_conf.mean(),
                    correct_conf.std(),
    ...
    def analyze_errors(self):
        ...
        misclassified_summary = misclassified[
            ["filename", "true_label", "predicted_label", "confidence", "num_segments"]
        ]
        misclassified_summary = misclassified_summary.sort_values(
            "confidence", ascending=False
        )
        misclassified_summary.to_csv(
            os.path.join(self.tables_dir, "misclassified_files.csv"), index=False
        )
        ...
        confusion_pairs.to_csv(
            os.path.join(self.tables_dir, "confusion_patterns.csv"), index=False
        )
    ...
    def analyze_segments(self):
        ...
        fig, ax = plt.subplots(figsize=(10, 6))
        self.df_success["num_segments"].value_counts().sort_index().plot(
            kind="bar", ax=ax, color="steelblue", edgecolor="black"
        )
        ax.set_xlabel("Number of Segments", fontsize=12)
        ax.set_ylabel("Frequency", fontsize=12)
        ax.set_title(
            "Distribution of Number of Segments per File",
            fontsize=14,
            fontweight="bold",
        )
        plt.tight_layout()
        ...
        segment_cols = [
            col for col in self.df_success.columns if col.startswith("predicted_label_")
        ]
        confidence_cols = [
            col
            for col in self.df_success.columns
            if col.startswith("confidence_") and not col == "confidence"
        ]
        ...
        if segment_cols:
            ...
            def calculate_consistency(row):
                ...
                if not segment_preds:
                    return np.nan
                return sum(1 for pred in segment_preds if pred == final_pred) / len(
                    segment_preds
                )

            self.df_success["segment_consistency"] = self.df_success.apply(
                calculate_consistency, axis=1
            )

            # Plot segment consistency distribution
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.hist(
                self.df_success["segment_consistency"].dropna(),
                bins=20,
                color="mediumpurple",
                alpha=0.7,
                edgecolor="black",
            ...
            ax.set_xlabel(
                "Segment Consistency (Proportion Agreeing with Final Prediction)",
                fontsize=12,
            )
            ax.set_ylabel("Frequency", fontsize=12)
            ax.set_title(
                "Distribution of Segment Prediction Consistency",
                fontsize=14,
                fontweight="bold",
            )
            plt.tight_layout()
            ...
            fig, ax = plt.subplots(figsize=(10, 6))
            correct_consistency = self.df_success[self.df_success["correct"]][
                "segment_consistency"
            ].dropna()
            incorrect_consistency = self.df_success[~self.df_success["correct"]][
                "segment_consistency"
            ].dropna()

            ax.hist(
                [correct_consistency, incorrect_consistency],
                bins=15,
                label=["Correct", "Incorrect"],
                color=["green", "red"],
                alpha=0.6,
            ...
            ax.set_ylabel("Frequency", fontsize=12)
            ax.set_title(
                "Segment Consistency: Correct vs Incorrect Predictions",
                fontsize=14,
                fontweight="bold",
            )
            ax.legend()
            plt.tight_layout()
            plt.savefig(
                os.path.join(self.plots_dir, "segment_consistency_by_correctness.png")
            )
            plt.close()

            # Average segment-level confidence
            if confidence_cols:
                self.df_success["avg_segment_confidence"] = self.df_success[
                    confidence_cols
                ].mean(axis=1)

                fig, ax = plt.subplots(figsize=(10, 6))
                ax.scatter(
                    self.df_success["avg_segment_confidence"],
                    self.df_success["confidence"],
                    c=colors,
                    alpha=0.5,
                    s=30,
                ...
                ax.set_title(
                    "Segment-Level vs File-Level Confidence",
                    fontsize=14,
                    fontweight="bold",
                )
                ax.legend(handles=legend_elements)
                plt.tight_layout()
                plt.savefig(
                    os.path.join(self.plots_dir, "segment_vs_file_confidence.png")
                )
                plt.close()
    ...
    def analyze_by_file_type(self):
        ...
        ft_df.to_csv(
            os.path.join(self.tables_dir, "performance_by_file_type.csv"), index=False
        )
        ...
        axes[0, 1].set_title(
            "F1-Score (Macro) by File Type", fontsize=12, fontweight="bold"
        )
        ...
        axes[1, 0].set_title(
            "Mean Confidence by File Type", fontsize=12, fontweight="bold"
        )
        ...
        axes[1, 1].set_title(
            "Mean Number of Segments by File Type", fontsize=12, fontweight="bold"
        )
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            for class_name in self.classes:
                if class_name in report_dict:
                    metrics = report_dict[class_name]
                    f.write(
                        f"{class_name:<20} {metrics['precision']:<12.4f} {metrics['recall']:<12.4f} "
            ...
            f.write("-" * 80 + "\n")
            f.write(
                f"Overall Mean Confidence: {self.df_success['confidence'].mean():.4f}\n"
            )
            f.write(
                f"Overall Median Confidence: {self.df_success['confidence'].median():.4f}\n"
            )
            f.write(
                f"Overall Std Confidence: {self.df_success['confidence'].std():.4f}\n"
            )

            if "correct" in self.df_success.columns:
                correct_conf = self.df_success[self.df_success["correct"]]["confidence"]
                incorrect_conf = self.df_success[~self.df_success["correct"]][
                    "confidence"
                ]
                f.write(
                ...
                f.write(
                    f"Incorrect Predictions - Mean Confidence: {incorrect_conf.mean():.4f}\n"
                )
            f.write("\n")
            ...
            f.write(
                f"Error Rate: {len(misclassified) / len(self.df_success) * 100:.2f}%\n\n"
            )
            ...
            f.write("-" * 80 + "\n")
            f.write(
                f"Mean Number of Segments: {self.df_success['num_segments'].mean():.2f}\n"
            )
            f.write(
                f"Median Number of Segments: {self.df_success['num_segments'].median():.0f}\n"
            )
            f.write(
                f"Min/Max Segments: {self.df_success['num_segments'].min():.0f} / {self.df_success['num_segments'].max():.0f}\n"
            )
            ...
            if "segment_consistency" in self.df_success.columns:
                ...
                f.write(
                    f"Segment Consistency (Incorrect): {incorrect_consistency:.4f}\n"
                )
            f.write("\n")

            # File type analysis
            if (
                "file_type" in self.df_success.columns
                and len(self.df_success["file_type"].unique()) > 1
            ):
                f.write("-" * 80 + "\n")
...
def main():
    ...
    print(
        f"✓ Review the summary report at: {os.path.join(output_folder, 'reports', 'analysis_summary_report.txt')}"
    )
