"""Module for audio data preprocessing: loading, segmentation, log-Mel spectrogram computation,
normalization, TFRecord serialization, and stratified train/val/test splitting with dataset-level normalization
"""
...
def apply_normalization(
    log_mel: np.ndarray, mean: np.ndarray, std: np.ndarray, per_bin: bool = True
) -> np.ndarray:
    """
...
def compute_dataset_statistics(
    ...
    # ========================================================================
    log.info(
        f"Spectrogram parameters: n_fft={n_fft}, hop_length={hop_length}, "
    ...
    for audio_path in tqdm(audio_paths, desc="Computing statistics"):
        ...
        try:
            ...
            log.warning(
                f"Error processing {audio_path} during statistics computation: {e}"
            )
            continue
    ...
    if per_bin:
        ...
        log.info(
            f"Computed per-bin statistics: mean shape {mean.shape}, std shape {std.shape}"
        )
    else:
...
def save_normalization_stats(
    ...
    log.info(
        f"Saved spectrogram parameters: n_fft={n_fft}, hop_length={hop_length}, "
        f"n_mels={n_mels}, min_db={min_db}"
    )
...
def stratified_split_indices(
    ...
    for cls in unique_classes:
        ...
        log.info(
            f"Class {cls}: {n_train} train, {n_val} val, {n_samples - n_train - n_val} test"
        )
...
def build_data_pipeline_01(
    ...
    for indices, split_dir, split_label, split_name in split_info:
        ...
        log.info(
            f"{split_name.capitalize()} set: {success_count} processed, {failed_count} skipped"
        )
